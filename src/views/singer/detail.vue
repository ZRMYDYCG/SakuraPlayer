...
function getSingerDesc(): void {
  const { id } = route.query
  reqSingerDesc({ id: Number(id) }).then((res) => {
    console.log(res)
  })
}
// 歌手粉丝
function genFans(): void {
  const { id } = route.query
  reqSingerFans({ id: Number(id) }).then((res) => {
...
function getSingHotSongs() {
  const { id } = route.query
  reqSingHotSongs({ id: Number(id) }).then((res) => {
  ...
  <MiniPlayOut>
    <div class="singer_detail">
      <van-nav-bar
        ...
        fixed
        placeholder
        :title="artist.name || '歌手详情'"
        @click-left="onClickLeft"
      />
      <ScrollBanner ref="scrollRef">
        <template #btn>
          <div v-if="artist.cover" class="top_content">
        ...
        <div class="container">
          <div class="box_container user_info">
            ...
            <div class="fansCnt">
              {{ formatCountNumber(fansData.fansCnt) }}粉丝
            </div>
            <div class="btn_wrapper">
              ...
              <van-button v-else size="small" block :loading="loading" round @click="subSinger(0)">
                取消
              </van-button>
            </div>
          </div>
          ...
          <div class="box_container hot_song">
            <div class="big_title">
              <div>热门歌曲</div>
              <div>
                <van-button round size="small" style="margin-right: 15px" @click="playAll">
                  播放全部
                </van-button>
              </div>
            </div>
            <div class="">
              ...
              <van-button round size="small" block @click="goAllSongs">
                查看更多
              </van-button>
            </div>
          </div>
          ...
          <div class="box_container">
            ...
            <van-row>
              <van-col v-for="item in albumList" :key="item.id" :span="8">
