...
let before: undefined | number // 分页
...
<template>
  <div class="conversation" :style="style">
    ...
    <div v-if="pullingDownLoading" class="loaing">
      <van-loading color="#e20001" />
    </div>
    <div class="scroll_wrapper" @click="hideTool">
      <Scroll ref="srcollRef" :probe-type="3" :stop-propagation="true" @pulling-down="pullingDown">
        <div class="list">
          <MsgItem v-for="item in list" :key="item.id" :msg-data="item" @preview-img="previewImg" />
        </div>
        <div class="end_el" />
      </Scroll>
    </div>
    <SendBox ref="sendBox" :to-user="uid" @change-height="refreshScroll" @send-success="sendSuccess" />
  </div>
</template>
