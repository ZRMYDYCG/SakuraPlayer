  ...
  <van-popup v-model:show="show" round append-to-body position="bottom">
    <div class="content">
      <div class="menu">
        ...
        <div class="finish" @click="createSheet">
          完成
        </div>
      </div>
      <van-form>
        <van-cell-group>
          <van-field v-model="formData.title" label="标题" placeholder="请输入歌单标题" required />
          <van-field label="公开">
            <template #input>
              <van-radio-group v-model="formData.public" direction="horizontal">
                <van-radio :name="true" checked-color="#e20001">
                  是
                </van-radio>
                <van-radio :name="false" checked-color="#e20001">
                  否
                </van-radio>
              </van-radio-group>
            </template>
