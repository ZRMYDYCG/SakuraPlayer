...
import { MusicType, PlayModeData } from '@/types/store/player'
...
function handler(lyric: LineLyric) {
  ...
  if (activeIndex.value > 5) {
    srcoll.value
    && srcoll.value.scrollToElemet(
      document.getElementsByClassName(`lyrcline${activeIndex.value - 5}`)[0] as HTMLElement,
      1000,
    )
  }
  else {
    srcoll.value
    && srcoll.value.scrollToElemet(document.getElementsByClassName(`lyrcline${0}`)[0] as HTMLElement, 1000)
  }
...
function getComment() {
  ...
  reqSongComment({ id: currentSong.value.id }).then((res) => {
...
function onPlayEnd() {
  console.log('播放结束')
  if (playMode.value === PlayModeData.list) {
    // 列表播放
    if (lyric) {
      lyric.stop()
      activeIndex.value = 0
  ...
  else if (playMode.value === PlayModeData.loop) {
    // 列表循环
    if (lyric) {
      lyric.stop()
      activeIndex.value = 0
  ...
  else if (playMode.value === PlayModeData.random) {
    // 随机播放
    if (lyric) {
      lyric.stop()
      activeIndex.value = 0
  ...
  else if (playMode.value === PlayModeData.single) {
    // 单曲循环
  ...
  <div class="bigPlayer" :class="{ activePlayer: showBigPlayer }">
    <div class="main_content">
      <div class="top-info">
        ...
        <div class="center">
          <div class="song_name">
            {{ currentSong.name ? currentSong.name : '歌曲名字' }}
          </div>
      ...
      <div class="center_music" @click="showLyric = !showLyric">
        <!-- 歌曲图片 -->
        <div v-show="!showLyric" class="disc" :class="{ disc_pause: !playing }">
        ...
        <div v-show="showLyric" class="lyric">
          <Scroll ref="srcoll" :probe-type="3" :stop-propagation="true">
            <div v-if="lyricLines.length" class="lyric_div">
              <div
                v-for="(item, index) in lyricLines"
                :key="index"
                class="lyrclines"
                :class="[`lyrcline${index}`, activeIndex == index ? 'lyrclactive' : '']"
              >
                {{ item.txt }}
              </div>
            </div>
      ...
      <div class="menu_wrapper">
        ...
        <div class="action_menu">
          <div v-if="currentSong.type !== MusicType.dj" class="action_menu_item" @click="likeSong">
            <i v-if="!isLike" class="iconfont icon-xihuan1" />
            <i v-else class="iconfont icon-xihuan redcolor" />
          </div>
          ...
          <div class="action_menu_item" @click="showComment = true">
            ...
            <i class="iconfont icon-31xiaoxi" />
          </div>
          <div class="action_menu_item">
        ...
        <div class="process">
          ...
          <div class="right_time">
            {{ formatMusicTime(currentSong.dt) }}
          </div>
        </div>
        <!-- 播放菜单 -->
        <div class="play_menu">
          ...
          <div
            v-if="playMode === PlayModeData.list"
            class="play_menu_item"
            @click="switchMode(PlayModeData.list)"
          >
            <i class="iconfont icon-shuzishunxu" />
          </div>
          <div
            v-if="playMode === PlayModeData.random"
            class="play_menu_item"
            @click="switchMode(PlayModeData.random)"
          >
            <i class="iconfont icon-24gl-shuffle" />
          </div>
          <div
            v-if="playMode === PlayModeData.loop"
            class="play_menu_item"
          ...
          <div
            v-if="playMode === PlayModeData.single"
            class="play_menu_item"
    ...
    <div class="player_bg" :style="bgStyle" />
    ...
    <CommentPopup
      ...
      :source-id="currentSong.id"
      :comment-type="currentSong.type == MusicType.dj ? CommentType.dj : CommentType.song"
    />
    <ShareFriend v-if="showShare" v-model:visible="showShare" :source="MsgType.song" :source-id="currentSong.id" />
  </div>
</template>
...
{{ singerName }}
      ...
      <div class="center_music" @click="showLyric = !showLyric">
        <!-- 歌曲图片 -->
        <div class="disc" v-show="!showLyric" :class="{ disc_pause: !playing }">
          <div class="m-song-turn animate__rotate" :class="{ rotate_pause: !playing }">
            <div class="m-song-rollwrap">
              <div class="m-song-img">
                <img class="u-img" :src="coverImg" alt="">
        ...
        <!-- 歌词 -->
        <div v-show="showLyric" class="lyric">
          <Scroll :probe-type="3" ref="srcoll" :stop-propagation="true">
            <div v-if="lyricLines.length" class="lyric_div">
              <div
                v-for="(item, index) in lyricLines"
                :key="index"
...
</template>.includes(String(currentSong.value.id))) {
    const url = `https://music.163.com/song/media/outer/url?id=${currentSong.value.id}.mp3`
    songUrl.value = url
  }
 else {
